...
def targetDataLoader(examples_training, labels_training,
    ...
    for j in range(17):
        ...
        for k in range(len(examples_training[j])):
            if k < training_cycle * 7:
                tgtExamples_personne_training.extend(examples_training[j][k])
                tgtLabels_gesture_personne_training.extend(labels_training[j][k])
    ...
    return tgt_list_train_dataloader
