    ...
    for sample_index in range(len(user_data)):
        ...
        while iter * stride + window_size <= emg_preprocessed.shape[1]:
            ...
            if sum(data_process.mav(emg_window)) < threshold:
                ...
                emg_spec = np.array(emg_spec, dtype=np.float32)     # (4, 8, 14)
                # print(emg_spec.shape)

                emg_spec = emg_spec.reshape(1, -1)          # (1, 448)
                # print(emg_spec.shape)
                # time.sleep(100)

                pred_label = new_lda.predict(emg_spec)      # <np.ndarray> [n]
                # print('True label: {}   Pred label: {}'.format(sample_label, int(pred_label)))
                # print(type(pred_label))
                # print()

                if pred_label < 7:
                    gesture_number_vector[int(pred_label)] = gesture_number_vector[int(pred_label)] + 1
