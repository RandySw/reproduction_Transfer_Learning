...
def format_data_to_train(vector_to_format):
    ...
    data_calculated = cal_wavelet.calculate_wavelet_dataset(dataset_example_formatted)
...
if __name__ == '__main__':
    ...
    examples, labels = read_data('../../EvaluationDataset', type="Test0")
    datasets = [examples, labels]
    np.save("formatted_datasets/saved_evaluation_dataset_test0.npy", datasets)

    examples, labels = read_data('../../EvaluationDataset', type="Test1")
    datasets = [examples, labels]
    np.save("formatted_datasets/saved_evaluation_dataset_test1.npy", datasets)

    datasets_training = np.load("formatted_datasets/saved_evaluation_dataset_training.npy",
                                encoding="bytes", allow_pickle=True)
    examples_training, labels_training = datasets_training

    datasets_test0 = np.load("formatted_datasets/saved_evaluation_dataset_test0.npy",
                             encoding="bytes", allow_pickle=True)
    examples_test0, labels_test0 = datasets_test0

    datasets_test1 = np.load("formatted_datasets/saved_evaluation_dataset_test1.npy",
                             encoding="bytes", allow_pickle=True)
