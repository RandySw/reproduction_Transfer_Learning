...
def butter_highpass(cutoff, fs, order=3):
    ...
    b, a = signal.butter(order, normal_cutoff,  btype='hp', analog=False)
    return b, a
...
def format_data_to_train(vector_to_format):
    ...
    # Apply the butterworth high pass filter (2Hz)
...
def shift_electrodes(examples, labels):
        ...
        for j in range(len(X_example)):
            if Y_example[j] == classe:
                if cwt_add == []:
                    ...
                    cwt_add += np.array(X_example[j][0])
        class_mean.append(np.argmax(np.sum(np.array(cwt_add), axis=0)))     # szy: find the most activated channel
    ...
    new_cwt_emplacement_right = ((np.array(0) - np.array(class_mean)) % 10)
    ...
    # We get the mean amount of shift and round it up tp get a discrete number representation how much we have to shift
    ...
    # Build the dataset of the candidate with the circular shift taken into account
    X_example = []
    Y_example = []
    for k in range(len(examples)):
...
def read_data(path):
    ...
    for candidate in range(12):
        ...
        for i in range(number_of_classes * 4):
            data_read_from_file = np.fromfile(path + '/Male' + str(candidate) + '/training0/classe_%d.dat'
                                              % i, dtype=np.int16)
    ...
    for candidate in range(7):
        ...
        examples = []
        for i in range(number_of_classes * 4):
            data_read_from_file = np.fromfile(path + '/Female' + str(candidate) + '/training0/classe_%d.dat'
                                              % i, dtype=np.int16)
            data_read_from_file = np.array(data_read_from_file, dtype=np.float32)
            dataset_example = format_data_to_train(data_read_from_file)
            examples.append(dataset_example)
